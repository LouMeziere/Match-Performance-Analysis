...
team_url = 'https://www.espn.co.uk/football/team/stats/_/id/17606/new-york-city-fc'
driver.get(team_url)
...
for idx, player_link in enumerate(player_links):
    ...
    try:
        ...
        if len(player_name_elements) >= 2:
            player_name = player_name_elements[0].text.strip() + " " + player_name_elements[1].text.strip()
    ...
    if table:
        ...
        if tbody:
            for row in tbody.find_all('tr', class_='Table__TR'):
                ...
                reordered_data.insert(0, player_name)
                
                # Append the reordered row to the data list
                data.append(reordered_data)

        # Create a DataFrame from the reordered data
        df = pd.DataFrame(data, columns=['Player'] + expected_headers)
        
        # Append this player's data to the all_player_data list
        all_player_data.append(df)
        
        print(f"Data for {player_name} has been collected")
    else:
        print(f"Table with specified class not found for player: {player_name}")
